...
"A moudle for getting the smallest prime factor of an integer."
...
if __name__ == '__main__':
    if len(sys.argv) != 2:
        sys.exit(
            sys.argv[0] + ": Expecting one command line argument -- the integer for which to get the smallest factor")
    n = int(sys.argv[1])
    if n < 1:
